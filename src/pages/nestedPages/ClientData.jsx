...
const ClientData = () => {
	...
	const sendOfferCart = () => {
		...
		formData.append('discount_expire_date', discount_expire_date ? moment(discount_expire_date).format('YYYY-MM-DD') : '');
	...
	return (
		<>
			...
			<div className='' open={true}>
				<Modal open={true} onClose={() => navigate('/Carts')} aria-labelledby='modal-modal-title' aria-describedby='modal-modal-description'>
													...
													<div className='row'>
														<div className='col-md-2 col-12 mb-md-0 mb-3'>
															<div className='client-date'>
																<img className='img-fluid' src={cartDetails?.user?.image} alt={cartDetails?.user?.name} />
																<div className='text'>
																	<div className='register-type mb-1'>{cartDetails?.user_type === 'store' ? '	التسجيل في المتجر' : '	التسجيل في المتجر'}</div>

																	<div className='register-date'>{moment(cartDetails?.created_at).format('DD-MM-YYYY')}</div>
																</div>
															</div>
														</div>
														<div className='col-md-8 col-12  align-self-center'>
															<div className='row mb-4 '>
																<div className='col-12 col-md-5'>
																	<div className='user-name '>{cartDetails?.user?.name}</div>
																</div>
															</div>
															<div className='user-info'>
																<div className='row user-info-row'>
																	<div className='col-md-2 col-12 mb-md-0 mb-3'>
																		<Location />
																		<span className='location '>{cartDetails?.user?.city?.name}</span>
																	</div>
																	<div className='col-md-4  col-12 mb-md-0 mb-3 d-flex justify-content-md-center align-items-center'>
																		<Phone />
																		<span className='location me-1'> {cartDetails?.user?.phonenumber} </span>
																	</div>
																	<div className='col-md-3 me-md-4 col-12 p-md-0'>
																		<Message />
																		<span className='location me-1'>{cartDetails?.user?.email} </span>
																	</div>
																</div>
															</div>
														</div>
											...
											<div className='userData-container'>
												<div className='container-title d-flex justify-content-between align-items-center'>
													<div className='tit-box'>
														...
														<span className='product-count me-2'>({cartDetails?.count} منتج)</span>
													</div>
												...
												{cartDetails?.cartDetail?.map((item) => (
													<div className='container-body products-details' key={item?.id}>
														<div className='row mb-md-4 mb-3'>
															...
															<div className='col-2'>
																<div className='count text-center'>{item?.product?.quantity}</div>
															</div>
															<div className='col-2'>
																<div className='price text-center'>{item?.price}</div>
															</div>
															<div className='col-3 d-flex justify-content-end'>
																<div className='total text-center'>{item?.sum} ر.س</div>
															</div>
														</div>
													</div>
												))}
												...
												<div className='container-body' style={{ height: '100%' }}>
													<div className='row'>
														<div className='col-12 mb-4'>
															<Switch
																onChange={() => setFree_shipping(!free_shipping)}
														...
														<div className='col-12 mb-4'>
															<Switch
																onChange={() => setOpenPercentMenu(!openPercentMenu)}
															...
															{openPercentMenu && (
																<div className='dropdown'>
																	<button
																		...
																		aria-expanded='false'
																		style={{ background: '#f4f5f7', minHeight: '58px', border: 'none', color: '#000', fontSize: '20px', borderRadius: '0' }}
																	>
																		نسبة الخصم
																	</button>
																	<ul className='dropdown-menu ul-percent-menu w-100' aria-labelledby='dropdownMenuButton1'>
																	<div className='percent-input-wrapper my-1'>
																		...
																		<div className='percent-menu w-100'>
																			<li className='w-100' style={{ cursor: 'pointer', padding: '16px' }} onClick={() => setDiscount_type('fixed')}>
																				مبلغ ثابت من المشتريات
																			</li>
																			<li className='w-100' style={{ cursor: 'pointer', padding: '16px' }} onClick={() => setDiscount_type('percent')}>
																				نسبة من المشتريات
																			</li>
																		</div>
										...
										<div className='mb-5 overflow-hidden'>
											...
											<div className='d-flex flex-row align-items-center gap-4'>
												<Editor
													...
													inDropdown={true}
													placeholder={
														<p style={{ fontSize: '20px', fontWeight: '400', color: '#011723', whiteSpace: 'normal' }}>
										...
										<div className='mb-md-5 mb-3'>
											<div className='create-offer-row'>
												...
												<div className='mb-md-0 mb-3 box total-discount-box'>
													...
													<input className='direction-ltr text-center' value={discount_total} onChange={(e) => setDiscount_total(e.target.value)} type='number' name='total-discount' id='total-discount' />
