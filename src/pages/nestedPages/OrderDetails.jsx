...
const OrderDetails = () => {
	...
	const { id } = useParams();
	const { fetchedData, loading, reload, setReload } = useFetch(`https://backend.atlbha.com/api/Store/orders/${id}`);
	...
	// 	axios
	// 		.get(`https://backend.atlbha.com/api/Store/orders/${id}`, {
	// 			headers: {
	...
	return (
		<>
			...
			<section className='order-details-page orders-pages p-md-3' id='report' ref={componentRef}>
					...
					<div className='row '>
						...
						<div className='col-md-6 col-12 d-flex justify-content-md-end justify-content-center'>
							<div className='order-number'>
								...
								<div className='number'>{loading ? 0 : fetchedData?.data?.orders?.order_number}</div>
							</div>
						</div>
					</div>
				</div>
											...
											<div className='row d-flex justify-content-center  '>
												<div className='col-md-3 col-12 mb-md-0 mb-3'>
													...
													<div className='order-data-row'>
														<span className='status'>{fetchedData?.data?.orders?.status}</span>
													</div>
												</div>
												<div className='col-md-3 col-12 mb-md-0 mb-3'>
													<div className='order-head-row '>
												...
												<div className='col-md-3 col-12 mb-md-0 mb-3'>
													...
													<div className='order-data-row'>
														<span>{fetchedData?.data?.orders?.total_price}</span>
													</div>
												</div>
											</div>
										</div>
												...
												<div className='col-md-2 col-12 mb-md-0 mb-3'>
													<div className='client-date'>
														<div className='img mb-2'>
															<img className=' img-fluid' src={fetchedData?.data?.orders?.user?.image} alt='client' />
														</div>
														<div className='text'>
															<div className='register-type mb-1'>التسجيل في المتجر</div>
															<div className='register-date'>{moment(fetchedData?.data?.orders?.user?.created_at).format('DD/MM/YYYY')}</div>
														</div>
													</div>
												</div>
													...
													<div className='row mb-md-4 mb-3'>
														<div className='col-md-6 col-12 mb-md-0 mb-3'>
															<div className='info-box'>
																...
																<span>{fetchedData?.data?.orders?.user?.name}</span>
															</div>
														</div>
														<div className='col-md-6 col-12'>
															<div className='info-box'>
																<Phone />
																<span style={{direction :'ltr'}}>{fetchedData?.data?.orders?.user?.phonenumber}</span>
															</div>
														</div>
													</div>
													<div className='row'>
														<div className='col-md-6 col-12 mb-md-0 mb-3'>
															<div className='info-box' style={{
																...
																gap: '30px',}}>
																<Message />
																<span>{fetchedData?.data?.orders?.user?.email}</span>
															</div>
														</div>
														<div className='col-md-6 col-12'>
															<div className='info-box'>
																<Location />
																<span>{fetchedData?.data?.orders?.user?.city?.name}</span>
															</div>
														</div>
													</div>
												</div>
								...
								<div className='order-details-box'>
									...
									<div className='px-md-3'>
												...
												<div id='collapseOne' className='accordion-collapse collapse ' aria-labelledby='headingOne' data-bs-parent='#accordionExample'>
													<div className='accordion-body'>
														<ul className='select-status p-0'>
															<li onClick={() => updateOrderStatus('new')}>جديد</li>
															<li onClick={() => updateOrderStatus('completed')}>مكتمل</li>
															<li onClick={() => updateOrderStatus('delivery_in_progress')}>جاري التجهيز</li>
															<li onClick={() => updateOrderStatus('ready')}>جاهز</li>
															<li onClick={() => updateOrderStatus('canceled')}>ملغي</li>
															<li onClick={() => updateOrderStatus('not_completed')}>غير مكتمل</li>
														</ul>
													</div>
												</div>
											</div>
										...
										<div className='order-action-box mb-3'>
											...
											<div className='action-icon'>
												<Pdf targetRef={componentRef} filename='report-details.pdf' x={0.5} y={0.5} scale={0.6}>
													{({ toPdf }) => <PDFIcon className='pdf-icon' onClick={toPdf} />}
												</Pdf>
											</div>
										</div>
										{/**
