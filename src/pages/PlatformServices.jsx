...
const PlatformServices = () => {
	const { fetchedData, loading, reload, setReload } = useFetch('https://backend.atlbha.com/api/Store/etlobhaservice/show');
	const { fetchedData: services } = useFetch('https://backend.atlbha.com/api/Store/selector/services');
	...
	useEffect(() => {
		setData({
			...
			activity: fetchedData?.data?.stores?.activity?.map(active => active?.name),
		})
		...
		<>
			...
			<section className='PlatformServices-page p-lg-3'>
				...
				<div className='head-category mb-md-4 mb-3'>
					<div className='row m-0'>
						...
						<div className='col-md-6 col-12 d-flex justify-content-end p-0'>
							<button className='delegate-request-btn' onClick={() => dispatch(openDelegateRequestAlert())}>
								طلب مندوب
							</button>
						</div>
					</div>
				...
				<div className='delegate-request-form'>
					<h5 className='form-name mb-md-5 mb-3'>قم بتقديم طلب بالخدمات التي تحتاجها</h5>
					{
						loading ?
							(
								<div className=''>
								...
								<form onSubmit={(e) => e.preventDefault()}>
									<div className='row align-items-center mb-md-4 mb-3'>
										...
										<div className='col-md-7 col-12'>
											<input value={data?.store_name} className='w-100' type='text' disabled />
										</div>
									</div>
									<div className='row align-items-center mb-md-4 mb-3'>
										<div className='col-md-4 col-12 d-flex justify-content-md-center mb-md-0 mb-2'>
												...
												<Select
													...
													renderValue={(selected) => {
														if (data?.services.length === 0) {
															return <span style={{ color: '#011723' }}>اختر خدمة أو أكثر</span>;
														}
														return selected.map((item) => {
															const result = services?.data?.services?.filter((service) => service?.id === parseInt(item));
												...
												>
													{services?.data?.services?.map((service, index) => (
														<MenuItem key={index} value={service?.id}>
															<Checkbox checked={data?.services?.indexOf(service?.id) > -1} />
