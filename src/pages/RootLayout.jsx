...
const RootLayout = () => {
	...
	const { isOpenVerifyModal, verificationStoreStatus } = useSelector((state) => state.VerifyModal);

	const { isOpenMaintenanceModeModal } = useSelector((state) => state.MaintenanceModeModal);
	const { isVerifyStoreAlertOpen } = useSelector((state) => state.VerifyStoreAlertModal);
	...
	useEffect(() => {
		if (verificationStoreStatus === 'لم يتم الطلب' || verificationStoreStatus === 'جاري التوثيق' || verificationStoreStatus === 'طلب جديد' ) {
				...
				<ThemeProvider theme={theme}>
					<CssBaseline>
						<div className='app'>
							...
							{isOpenCelebrityMarketingModal && <CelebrityMarketingModal></CelebrityMarketingModal>}
