...
const MainInformation = () => {
	...
	const { fetchedData, loading, reload, setReload } = useFetch('https://backend.atlbha.com/api/Store/setting_store_show');
	...
	const settingsStoreUpdate = () => {
		...
		axios
			.post(`https://backend.atlbha.com/api/Store/setting_store_update`, formData, {
				headers: {
			...
			<div className='main-info-form'>
				{loading ? (
					...
					<div className='row'>
							...
							<div className='row d-flex justify-content-center align-items-center mb-3'>
								<div className='col-lg-6 col-12'>
									...
									<div className='upload-logo-set d-flex justify-content-center align-items-center flex-column'>
										{/** Upload Image  */}
										<ImageUploading value={storeLogo} onChange={onChangeStoreLogo} dataURLKey='data_url' acceptType={['jpg', 'png', 'jpeg']}>
											{({ imageList, onImageUpload, dragProps }) => (
												...
												<Fragment>
													...
													<div className='upload-image-wrapper'>
														{storeLogo[0] && (
															<div className='upload-image-bx mb-2'>
																<img src={storeLogo?.[0]?.data_url || storeLogo?.[0]} alt={''} className='img-fluid' />
															</div>
														)}
													</div>
													...
													{settingErr?.logo && <span className='fs-6 w-100 text-danger'>{settingErr?.logo}</span>}
												</Fragment>
											)}
										</ImageUploading>
									</div>
							...
							<div className='row d-flex justify-content-center align-items-center'>
								<div className='col-lg-8 col-12'>
									<div className='domain-name d-flex align-content-center justify-content-between'>
										...
										<input className='' type='text' name='domain' value={domain} onChange={(e) => setDomain(e.target.value)} />
									</div>

									{settingErr?.domain && <span className='fs-6 w-100 text-danger'>{settingErr?.domain}</span>}
								</div>
							</div>
						</div>
						<div className='col-12 mb-3'>
							<div className='row d-flex justify-content-center align-items-center'>
								<div className='col-lg-8 col-12'>
									<div className='select-country'>
										...
										<Select
											...
											renderValue={(selected) => {
												...
												const result = countryList?.data?.countries?.filter((item) => parseInt(item?.id) === parseInt(selected));
									...
									{settingErr?.country_id && <span className='fs-6 w-100 text-danger'>{settingErr?.country_id}</span>}
								</div>
							</div>
						</div>
						<div className='col-12 mb-3'>
							<div className='row d-flex justify-content-center align-items-center'>
								<div className='col-lg-8 col-12'>
									<div className='select-country'>
										...
										<Select
											...
											renderValue={(selected) => {
												...
												const result = citiesList?.data?.cities?.filter((item) => parseInt(item?.id) === parseInt(selected));
									...
									{settingErr?.city_id && <span className='fs-6 w-100 text-danger'>{settingErr?.city_id}</span>}
								</div>
							</div>
						</div>
						...
						<div className='col-12 mb-3'>
							{/** Upload Icon row */}
							<div className='row d-flex justify-content-center align-items-center' style={{ cursor: 'pointer' }}>
								<div className='col-lg-8 col-12'>
									<div className='select-country'>
										<label htmlFor='upload-icon' className='upload-icon-label'>
											ايقونة تبويب المتجر في المتصفح
										</label>
										<div>
											<ImageUploading value={storeIcon} onChange={onChangeSelectIcon} dataURLKey='data_url' acceptType={['jpg', 'png', 'jpeg']}>
												{({ onImageUpload, dragProps }) => (
													<div
														...
														{...dragProps}
													>
														<div style={{ width: '35px', height: '35px' }}>{storeIcon[0] && <img className='img-fluid' src={storeIcon[0].data_url || storeIcon[0]} alt='' style={{ objectFit: 'contain' }} />}</div>
									...
									{settingErr?.icon && <span className='fs-6 w-100 text-danger'>{settingErr?.icon}</span>}
								</div>
							</div>
						</div>
						...
						<div className='col-12 mb-5'>
							<div className='row d-flex justify-content-center align-items-center'>
								<div className='col-lg-8 col-12'>
									...
									{settingErr?.description && <span className='fs-6 w-100 text-danger'>{settingErr?.description}</span>}
								</div>
							</div>
						</div>
