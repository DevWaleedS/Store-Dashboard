...
const MaintenanceModeModal = () => {
	...
	const { fetchedData, reload, setReload } = useFetch('https://backend.atlbha.com/api/Store/maintenance');
	...
	const { isOpenMaintenanceModeModal } = useSelector((state) => state.MaintenanceModeModal);
	...
	const UpdateMaintenanceMode = (data) => {
		...
		formData.append('status', maintenanceStatus === true ? 'active' : 'not_active');

		axios
			.post(`https://backend.atlbha.com/api/Store/updateMaintenance`, formData, {
				headers: {
										...
										<div className='row maintenance-modal-form mx-0'>
											<div className='col-12 mb-3'>
												<div className='modal-input-group'>
													...
													/>
													<br />
													<span className='fs-6 text-danger'>{dataError?.title}{errors?.title && errors.title.message}</span>
												</div>
											</div>
											<div className='col-12 mb-4'>
												<div className='modal-input-group'>
													...
													></textarea>
													<br />
													<span className='fs-6 text-danger'>{dataError?.message}{errors?.message && errors.message.message}</span>
												</div>
											</div>
										</div>
									</div>
...
const MaintenanceMode = () => {
	return <Fragment>{ReactDom.createPortal(<MaintenanceModeModal />, document.getElementById('maintenance_mode_modal'))}</Fragment>;
};
