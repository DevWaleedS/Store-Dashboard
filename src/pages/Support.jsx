...
const Support = () => {
	...
	const { fetchedData, loading, reload, setReload } = useFetch('https://backend.atlbha.com/api/Store/technicalSupport');
	const [search, setSearch] = useState('');
	let Technicalsupports = fetchedData?.data?.Technicalsupports;
	if (search !== '') {
		Technicalsupports = fetchedData?.data?.Technicalsupports?.filter((item) => item?.title?.toLowerCase()?.includes(search?.toLowerCase()));
	} else {
