				...
				{numSelected > 0 && (
					<div>
						<Tooltip
							...
							onClick={() => {
								setNotificationTitle('سيتم حذف جميع المستخدمين وهذةالخظوة غير قابلة للرجوع');
						...
						<Tooltip
							...
							onClick={() => {
								setNotificationTitle('سيتم تغيبر حالة جميع المستخدمين التي قمت بتحديدهم');
...
export default function UserAndManagementTable({ data, loading, reload, setReload }) {
	...
	const changeUserStatus = (id) => {
		axios
			.get(`https://backend.atlbha.com/api/Store/userchangeSatusall?id[]=${id}`, {
				headers: {
	...
	useEffect(() => {
		if (confirm && actionTitle === 'Delete') {
			...
			axios
				.get(`https://backend.atlbha.com/api/Store/userdeleteall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
		...
		if (confirm && actionTitle === 'changeStatus') {
			const queryParams = selected.map((id) => `id[]=${id}`).join('&');
			axios
				.get(`https://backend.atlbha.com/api/Store/userchangeSatusall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
														...
														<TableCell align='right'>
															<div className='actions d-flex justify-content-evenly'>
																...
																<span>
																	<DeletteIcon
																		onClick={() => {
																			setActionDelete('سيتم حذف المستخدم وهذة الخطوة غير قابلة للرجوع');
																			setDeleteMethod('get');
																			setUrl(`https://backend.atlbha.com/api/Store/userdeleteall?id[]=${row?.id}`);
																		}}
																		style={{
																			cursor: 'pointer',
																			color: 'red',
