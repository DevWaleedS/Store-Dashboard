			...
			<div className='row mb-3'>
				<div className='col-12'>
					<div className='comp-title d-flex justify-content-between '>
						<h4>أحدث الطلبات </h4>
						<BsArrowLeft className='arrow-style' onClick={() => navigate('Orders')} />
					</div>
				</div>
			</div>
					...
					<table className='table table-borderless orders-table text-center'>
						<thead>
							<tr>
								...
								<th scope='col' style={{ whiteSpace: 'nowrap', width: '85px', textAlign: 'center' }}>
									رقم الطلب
								</th>
								<th scope='col' style={{ whiteSpace: 'nowrap', width: '200px' }}>
									...
									<tr>
										<td>
											<div style={{ whiteSpace: 'normal', width: '40px' }}>
												<img style={{ width: '100%' }} className=' rounded-circle ' src={order?.user?.image} alt={order?.user?.name} />
											</div>
										</td>

										<td className=' text-end pe-3 text-overflow' style={{ whiteSpace: 'nowrap', width: '85px', textAlign: 'center' }}>
											{order?.order_number}
										</td>
										<td className='text-overflow' style={{ whiteSpace: 'nowrap', width: '200px' }}> {order?.user?.name}</td>
										<td>
											<span
										...
										<td className=' text-center action-icon'>
											<div className='dropdown'>
												...
												<ul className='dropdown-menu new-orders-dropdown-menu'>
													<li>
														<Link className='dropdown-item' to={`/Orders/OrderDetails/${order?.id}`}>
