...
const ROBOTS_REGEX = /^https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_\+.~#?&\/=]*)$/;

const SeoWeight = () => {
	// to get all  data from server
	const { fetchedData, loading, reload, setReload } = useFetch(`https://backend.atlbha.com/api/Store/seo`);
	...
	const index_page_title = fetchedData?.data?.Seo?.map((item) => item?.index_page_title) || [];
	const index_page_description = fetchedData?.data?.Seo?.map((item) => item?.index_page_description) || [];
	const show_pages = fetchedData?.data?.Seo?.map((item) => item?.show_pages) || [];
	const keyWords = fetchedData?.data?.Seo?.map((item) => item?.key_words?.[0]) || [];
	...
	useEffect(() => {
		const storeDescValidation = DESC_REGEX.test(seoValue?.index_page_description);
			...
			.then((res) => {
				if (res?.data?.success === true && res?.data?.data?.status === 200) {
					...
					setDataError({
						...
						index_page_description: res?.data?.message?.en?.index_page_description?.[0],
						...
						<Fragment>
							<div className='col-12 mb-md-4 mb-3'>
								<div className='input-bx'>
									...
									/>
									<br />
									<span className='fs-6 text-danger'>{dataError?.index_page_title}</span>
									<p
										id='pageTitle'
										className={pageTitleFocus && seoValue?.index_page_title && !validPAGETITLE ? ' d-block wrong-text ' : 'd-none'}
							...
							<div className='col-12 mb-4'>
								<div className='input-bx'>
									...
									/>
									<br />
									<span className='fs-6 text-danger'>{dataError?.index_page_description}</span>
									<p
										id='pageDesc'
										className={pageDescFocus && seoValue?.index_page_description && !validPAGEDESC ? ' d-block wrong-text ' : 'd-none'}
										style={{ color: 'red', padding: '10px',fontSize:'1rem' }}
									>
							...
							<div className='col-12 mb-4'>
								<div className='input-bx'>
									<TagsInput value={keyWord} onChange={setKeyWord} name='key_words' placeHolder='الكلمات المفتاحية' />
									<EditIcon className='key-word-icons' />
									<br />
									<span className='fs-6 text-danger'>{dataError?.key_words}</span>
								</div>
							</div>
							...
							<div className='col-12 mb-5'>
								<div className='input-bx'>
									...
									<RadioGroup aria-labelledby='demo-controlled-radio-buttons-group' defaultValue='short_link' name='controlled-radio-buttons-group' value={showPageValue} onChange={handleShowPages}>
										<div className='seo-form-check mb-2'>
											<FormControlLabel className='mb-md-0 mb-3' name='short_link' value='short_link' control={<Radio />} />
											<label className='seo-form-check-label me-md-2' htmlFor='shortLink'>
												<span>رابط مختصر</span>
												<span className='link me-2'>(https://sample.utlopha.com) </span>
											</label>
										</div>
										<div className='seo-form-check '>
											<FormControlLabel className='mb-md-0 mb-3' name='name_link' value='name_link' control={<Radio />} />
											<label className='seo-form-check-label  me-md-2' htmlFor='normalLink'>
								...
								<br />
								<span className='fs-6 text-danger'>{dataError?.show_pages}</span>
							</div>
						...
						<div className='row p-3'>
							<div className='col-12 mb-5'>
								<div className='input-bx'>
									...
									<p
										id='pageDesc'
										className={pageLinkFocus && updateLinkValue && !validPageLink ? ' d-block wrong-text ' : 'd-none'}
										style={{ color: 'red', padding: '10px',fontSize:'1rem' }}
									>
						...
						<div className='row p-3'>
							<div className='col-12 mb-4'>
								<div className='input-bx'>
									...
									<p
										id='pageDesc'
										className={pageRobotsFocus && updateRobotsValue && !validPageRobots ? ' d-block wrong-text ' : 'd-none'}
										style={{ color: 'red', padding: '10px',fontSize:'1rem' }}
									>
