				...
				{numSelected > 0 && (
					<div>
						<Tooltip
							...
							onClick={() => {
								setNotificationTitle('سيتم حذف جميع الكوبونات وهذةالخظوة غير قابلة للرجوع');
...
export default function CouponTable({ data, loading, reload, setReload }) {
	...
	const changeProductStatus = (id) => {
		axios
			.get(`https://backend.atlbha.com/api/Store/couponchangeSatusall?id[]=${id}`, {
				headers: {
	...
	useEffect(() => {
		if (confirm && actionTitle === 'Delete') {
			...
			axios
				.get(`https://backend.atlbha.com/api/Store/coupondeleteall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
		...
		if (confirm && actionTitle === 'changeStatus') {
			const queryParams = selected.map((id) => `id[]=${id}`).join('&');
			axios
				.get(`https://backend.atlbha.com/api/Store/couponchangeSatusall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
											...
											?.map((row, index) => {
												...
												return (
													<TableRow hover role='checkbox' aria-checked={isItemSelected} tabIndex={-1} key={index} selected={isItemSelected}>
														...
														<TableCell align='right'>{row?.discount_type}</TableCell>
														<TableCell align='right'>{moment(row?.expire_date).format('YYYY-MM-DD')}</TableCell>
														...
														<TableCell align='center'>
															<span
																...
																className='status'
																style={{
																	backgroundColor: row?.status === 'نشط' ? '#e0ffea' : '#ebebeb',
																	color: row?.status === 'نشط' ? '#3ae374' : '#a7a7a7',
																	borderRadius: '16px',
																	padding: '5px 25px',
																	fontWeight: 500,
																}}
																...
																<span>
																	<DeletteIcon
																		onClick={() => {
																			...
																			setDeleteMethod('get');
																			setUrl(`https://backend.atlbha.com/api/Store/coupondeleteall?id[]=${row?.id}`);
																		}}
																		style={{
																			cursor: 'pointer',
																			color: 'red',
