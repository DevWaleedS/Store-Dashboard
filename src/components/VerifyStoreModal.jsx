...
const VerifyStore = () => {
	...
	const { fetchedData, loading } = useFetch('https://backend.atlbha.com/api/Store/verification_show');
	...
	useEffect(() => {
		if (fetchedData?.data?.stores) {
			setVerificationStatus(fetchedData?.data?.stores?.map((store) => store?.verification_status)[0]);
		}
	...
	return (
		<div>
			{verificationStatus === '' || null ? null : (
				...
				>
					<Box sx={style}>
						<Typography id='modal-modal-description' sx={{ mt: 2, '@media(max-width:768px)': { mt: 0 } }}>
...
const VerifyStoreModal = () => {
	return <Fragment>{ReactDom.createPortal(<VerifyStore />, document.getElementById('verifay_store_modal'))}</Fragment>;
};
