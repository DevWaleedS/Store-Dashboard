...
export default function DelegateTable({ cityId }) {
	const { fetchedData, loading } = useFetch(`https://backend.atlbha.com/api/Store/marketerRequest/${cityId}`);
											...
											fetchedData?.data?.marketers?.length === 0 ?
												...
												fetchedData?.data?.marketers?.map((row, index) => (
													<TableRow hover role='checkbox' tabIndex={-1} key={index}>
														<TableCell component='th' scope='row' align='center' sx={{ width: '100px' }}>
														...
														<TableCell align='center'>
															<div className='cate-prim d-flex align-items-center justify-content-start' style={{ width: '350px', paddingRight: ' 70px ' }}>
																<img src={row?.image} alt='img' className=' rounded-circle' />
																<span className='me-3'>{row?.name}</span>
															</div>
														</TableCell>
														<TableCell align='center' dir='ltr'>{row?.phonenumber}</TableCell>

														<TableCell align='center'>{row?.city?.name}</TableCell>
														<TableCell align='right'>
															<div className='actions d-flex justify-content-center gap-1'>
																<IoMdStar style={{ color: '#B6BE34', fontSize: '20px' }} />
																<IoMdStar style={{ color: '#B6BE34', fontSize: '20px' }} />
																<IoMdStar style={{ color: '#B6BE34', fontSize: '20px' }} />
																<IoMdStar style={{ color: '#B6BE34', fontSize: '20px' }} />
																<IoMdStar style={{ color: '#B6BE34', fontSize: '20px' }} />
