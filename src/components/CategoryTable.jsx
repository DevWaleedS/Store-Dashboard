				...
				{numSelected > 0 && (
					<div>
						<Tooltip
							...
							onClick={() => {
								setNotificationTitle('سيتم حذف جميع التصنيفات وهذةالخطوة غير قابلة للرجوع');
...
export default function EnhancedTable({ fetchedData, loading, reload, setReload }) {
	...
	const changeCategoryStatus = (id) => {
		axios
			.get(`https://backend.atlbha.com/api/Store/categoryStorechangeSatusall?id[]=${id}`, {
				headers: {
	...
	useEffect(() => {
		if (confirm && actionTitle === 'Delete') {
			...
			axios
				.get(`https://backend.atlbha.com/api/Store/categoryStoredeleteall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
		...
		if (confirm && actionTitle === 'changeStatus') {
			const queryParams = selected.map((id) => `id[]=${id}`).join('&');
			axios
				.get(`https://backend.atlbha.com/api/Store/categoryStorechangeSatusall?${queryParams}`, {
					headers: {
						'Content-Type': 'application/json',
						Authorization: `Bearer ${cookies.access_token}`,
					},
								...
								<Fragment>
									{fetchedData?.length === 0 ?
										...
										fetchedData?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((row, index) => {
											...
											return (
												<TableRow style={{ backgroundColor: row?.store === null ? '#dfe2aa' : '' }} hover role='checkbox' aria-checked={isItemSelected} tabIndex={-1} key={index} selected={isItemSelected}>
													...
													<TableCell align='center text-overflow' sx={{ width: '73px' }}>
														{row?.number}
													</TableCell>
													<TableCell>
														<div className='cate-prim d-flex align-items-center justify-content-start' style={{ width: ' 300px', paddingRight: '60px' }}>
															<img src={row?.icon} alt={row?.name} style={{ minWidth: ' 40px', maxWidth: '40px', height: '40px', objectFit: 'contain' }} />
															<span className='me-3 text-overflow'>{row?.name}</span>
														</div>
													</TableCell>
													<TableCell align='center text-overflow'>{row?.countsubcategory}</TableCell>
													...
													<TableCell align='right'>
														<div className='sub-categories'>
															...
															{row?.subcategory?.length > 3 && <span>...</span>}
														</div>
													</TableCell>

													<TableCell align='center'>
														<div className='form-check form-switch' style={{ margin: '0 auto' }}>
															<Switch
																disabled={row?.store === null ? true : false}
													...
													<TableCell align='right'>
														<div className='actions d-flex justify-content-evenly'>
															<span style={{ pointerEvents: row?.store === null ? 'none' : '' }}>
															...
															<span>
																<DeletteIcon
																	onClick={() => {
																		setActionDelete('سيتم حذف التصنيف وهذة الخطوة غير قابلة للرجوع');
																		setDeleteMethod('get');
																		setUrl(`https://backend.atlbha.com/api/Store/categoryStoredeleteall?id[]=${row?.id}`);
																	}}
																	style={{
																		pointerEvents: row?.store === null ? 'none' : '',
																		cursor: 'pointer',
																		color: 'red',
																		fontSize: '1.2rem',
																	}}
